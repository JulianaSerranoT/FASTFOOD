...
router.post('/ordenes', async (req, res) => {
    ...
    // Creamos la orden
    const response = await axios.get(`http://usuarios:3005/usuarios/${usuario}`);
...
async function calcularTotal(items) {
    ...
    for (const producto of items) {
        const response = await axios.get(`http://productos:3003/productos/${producto.id}`);
...
async function actualizarInventario(items) {
    for (const producto of items) {
        ...
        await axios.put(`http://productos:3003/productos/${producto.id}`, {
